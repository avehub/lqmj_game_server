    ...
    async def post(self, req: Request):
        ...
        if proxy_login_type == ProxyLoginType.WECHAT_MP_LOGIN:
            ...
            if not status:
                ...
                self.answer(StaCode.EXTERNAL_ERR, {}, hint='登录出错,请重试!')
            q_params = {
                "unionid":   wechat_map_user_data.get("unionid") if  wechat_map_user_data.get("unionid") else 123,
                "platform__in": h5_app,
            }
            u_info = await BaseUserRC.cache_by_unique(q_params,  BaseUserRC.KEY_UNION_ID)
            if not u_info:
                return self.answer(StaCode.FAIL, hint="微信不存在!")
            query_param = {"id": u_info.get("id")}
            # TODO union 查询用户是否注册
