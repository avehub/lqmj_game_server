...
wx.error((res) => {
    console.error("------------------------wx on error---------------------------");
    // alert(res.errMsg);  //打印错误消息。
    console.error(res.errMsg);
    let sdk = window.LQWeChatSdk;
    if (sdk && sdk.error) {
        sdk.isError = true;
        sdk.error(res);
    }
    ...
    try {
        ...
        wx.getNetworkType({
            success: function (res) {
                ...
                console.log("微信获取到网络类型：", networkType)
            }
        });
...
(function () {
    ...
    //注册微信分享
    const setShareInfo = (url, title, img, desc, onSuccess, onFailed) => {
        try {
            console.log("wx 设置分享 URL", url, "title", title);
            url = decodeURIComponent(url);//坑 url使用服务器签名时因为多个参数需要ecodeURIComponent(url)  那么这里调用分享时就一定要decodeURIComponent(url) 否则报must be in js...
            if (!wx) {
                console.log("没有wx对象");
                return;
            }
            wx.updateAppMessageShareData({
                ...
                imgUrl: img, // 分享图标
                success: () => {
                    console.log("设置发送给好友 success");
                    if (onSuccess) onSuccess();
                },
                fail: (err) => {
                    console.error("设置微信分享失败err", err);
    ...
    LQWeChatSdk.getLocation = getLocation;
    LQWeChatSdk.setShareInfo = setShareInfo;
})(LQWeChatSdk || (LQWeChatSdk = {}));