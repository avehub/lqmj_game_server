...
from ..base.base_leisure_service import LeisureService
...
class MatchServer(BaseServer, LeisureService):
    ...
    async def __rev_match_leisure(self, uid, data):
        ...
        # -- snip -- 2025/5/12 限制下篇入场条件
        # if cs_enum == ServiceEnum.C_MONSTER_SEQUEL:
        #     r_conf = await ConfJsonRC.cache_conf_data_by_pk(ConfJsonRC.CONF_RANKING)
        #     if ranking_conf.get("level") < r_conf.get("seq_limit_level", 16):
        #         hint = r_conf.get("seq_limit_hint") or "境界达到证道天仙V后才可以进入下篇"
