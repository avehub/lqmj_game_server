...
class MoveGenerator:
    ...
    def match_long_qi_dui(self, *args):
        ...
        """
        if not self.magic_card:
            return self.calc_xts_by_long_qi_dui_normal(*args)
    ...
    def calc_xl_normal_build_types(self):
        """
        计算卡牌组成类型及向听数
        """
        ...
        # 2.3 计算四喜财向听数及最佳出牌
        sxc_case = [
            len(args[2]) == self.xqd_flag_len + 1,
        ...
        if self.hand_cards_len == 14:
            # 3.1 计算七对向听数及最佳出牌
            if len(args[1]) > self.xqd_flag_len - 1:
    ...
    def calc_normal_build_types(self):
        ...
        # todo: 2.14张 -> 大对子/小七对/龙七对向听数及最佳出牌
        if self.hand_cards_len == 14:
            # 2.1 大对子
            ddz_case = [
                len(args[1] + args[2]) > self.xqd_flag_len,
                len(args[1]) > self.ddz_flag_len,
                len(args[2]) > self.ddz_flag_len
            ]
            if any(ddz_case):
                xts2, best_cards2 = self.cal_xts_da_dui_zi_normal(*args)
                all_played_cards.extend(best_cards2)
                all_xts_cards.append(("ddz", xts2, best_cards2))
                LOG_PRINT and print(f"大对子向听数: {xts2}, 最优出牌: {best_cards2}")
                LOG_PRINT and print()
            ...
            if len(args[1]) > self.xqd_flag_len:
                xts3, best_cards3 = self.cal_xts_by_qi_dui_normal(*args)
                all_played_cards.extend(best_cards3)
                all_xts_cards.append(("xqd", xts3, best_cards3))
                LOG_PRINT and print(f"小七对向听数: {xts3}, 最优出牌: {best_cards3}")
                LOG_PRINT and print()

            if len(args[1]) > self.ddz_flag_len and len(args[2]) == 1:
                xts4, best_cards4 = self.calc_xts_by_long_qi_dui_normal(*args)
                all_played_cards.extend(best_cards4)
                all_xts_cards.append(("xqd", xts4, best_cards4))
                LOG_PRINT and print(f"龙七对向听数: {xts4}, 最优出牌: {best_cards4}")
                LOG_PRINT and print()

        # todo: 3.11张大对子
        elif self.hand_cards_len == 11:
            if len(args[1] + args[2]) + len(self.split_piles()) > self.ddz_flag_len:
                xts5, best_cards5 = self.cal_xts_da_dui_zi_normal(*args)
                all_played_cards.extend(best_cards5)
                all_xts_cards.append(("ddz", xts5, best_cards5))
                LOG_PRINT and print(f"11张大队子向听数: {xts5}, 最优出牌: {best_cards5}")
                LOG_PRINT and print()

        # todo: 4.<11张大队子
        elif self.hand_cards_len < 11:
            ddz_case_lt_11 = [
                len(args[1] + args[2]) + len(self.split_piles()) > self.ddz_flag_len,
                len(args[1]) + len(self.split_piles()) > self.ddz_flag_len,
                len(args[2]) + len(self.split_piles()) > self.xqd_flag_len
            ]
            if any(ddz_case_lt_11):
                xts6, best_cards6 = self.cal_xts_da_dui_zi_normal(*args)
                all_played_cards.extend(best_cards6)
                all_xts_cards.append(("ddz", xts6, best_cards6))
                LOG_PRINT and print(f"小11张大队子向听数: {xts6}, 最优出牌: {best_cards6}")
                LOG_PRINT and print()

        return all_played_cards, all_xts_cards, best_cards1

    def calc_xl_qing_yi_se_build_types(self):
        """
        计算清一色卡牌组成类型及向听数
        """
        all_played_cards, all_xts_cards = [], []
    ...
    def calc_qing_yi_se_build_types(self):
        ...
        if self.hand_cards_len == 14:
            ...
            if len(args[1]) > self.xqd_flag_len:
                ...
                all_played_cards.extend(best_cards3)
                all_xts_cards.append(("xqd", xts3, best_cards3))
                LOG_PRINT and print(f"清一色 -> 小七对向听数: {xts3}, 最优出牌: {best_cards3}")
                LOG_PRINT and print()
            ...
            if len(args[1]) > self.ddz_flag_len and len(args[2]) == 1:
                ...
                all_played_cards.extend(best_cards4)
                all_xts_cards.append(("xqd", xts4, best_cards4))
                LOG_PRINT and print(f"清一色 -> 龙七对向听数: {xts4}, 最优出牌: {best_cards4}")
                LOG_PRINT and print()
        ...
        # todo: 3.11张大对子
            ...
            if len(args[1] + args[2]) + len(self.split_piles()) > self.ddz_flag_len:
                ...
                all_played_cards.extend(best_cards5)
                all_xts_cards.append(("ddz", xts5, best_cards5))
                LOG_PRINT and print(f"清一色 -> 11张大队子向听数: {xts5}, 最优出牌: {best_cards5}")
                LOG_PRINT and print()
        ...
        # todo: 4.<11张大队子
            ...
            ddz_case_lt_11 = [
                len(args[1] + args[2]) + len(self.split_piles()) > self.xqd_flag_len,
                len(args[1]) + len(self.split_piles()) > self.xqd_flag_len,
                len(args[2]) + len(self.split_piles()) > self.xqd_flag_len
            ]
            if any(ddz_case_lt_11):
                xts6, best_cards6 = self.cal_xts_da_dui_zi_normal(*args)
                all_played_cards.extend(best_cards6)
                all_xts_cards.append(("ddz", xts6, best_cards6))
                LOG_PRINT and print(f"清一色 -> 小于11张大队子向听数: {xts6}, 最优出牌: {best_cards6}")
                LOG_PRINT and print()
    ...
    def make_qing_yi_se_type(self, curr_cards_res):
        ...
        if self.left_count > self.modify_flag:
            ...
            if CardType.LAI_ZI in tmp_hand_cards:
                tmp_hand_cards.remove(CardType.LAI_ZI)
    ...
    def control_play_card(best_cards, xts_yxp_cards):
        ...
        return int(UtilsTool.random_choice_num([best_card, other_cards], [0.7, 0.3]))
    ...
    def cal_xts_ping_hu(self, *args, need_mz=None):
        ...
        # todo: 存在癞子牌
            ...
            best_cards = self.calc_lpy_by_ping_hu_lai_zi(deduplicate)
    ...
    def calc_lpy_by_ping_hu_lai_zi(self, deduplicate):
        """
        ...
        for i, path in enumerate(deduplicate):
            ...
            best_cards, yxp_count = self.calc_lai_zi_by_xts_cards(deduplicate)
    ...
    def calc_lai_zi_by_xts_cards(self, optimal_path):
        """
        计算老牌友麻将癞子卡牌
        根据胡牌类型计算有效牌
            1.平胡
        ...
        for idx, cards in enumerate(optimal_path):
            ...
            for card in optimal_path[idx]:
                # 判断癞子是否存在，组合成2张合法卡牌
                if CardType.LAI_ZI in cards:
                    if card != CardType.LAI_ZI:
                        card1_val = MoveGenerator.get_value(card)
                        if card1_val != 1:
                            yxp_cards.add(card - 1)
                        if card1_val != 9:
                            yxp_cards.add(card + 1)
    ...
    def calc_xts_da_dui_zi_lai_zi(self, *args):
        ...
        # todo: 大对子牌型时，存在单张卡牌，优先选择单张卡牌为最优出牌
        min_count = 0
        best_cards = []
        # 存在单张时，择优选择单张为最佳出牌
        if one_list:
        ...
        # 找不到最优出牌，说明单张，对子中不存在，则随机打一张
            ...
            if CardType.LAI_ZI in self.hand_cards:
                self.hand_cards.remove(CardType.LAI_ZI)
        ...
        if best_cards and len(best_cards) > 1:
            if CardType.LAI_ZI in best_cards:
                best_cards.remove(CardType.LAI_ZI)
    ...
    def calc_xts_by_long_qi_dui_normal(self, *args):
        """
        未持有癞子，龙七对
        """
