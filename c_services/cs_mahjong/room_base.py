...
class Room(BaseCardRoom):
    ...
    async def deal_cards(self):
        ...
        if self.__four_card_bao_ting:
            return self.__start_bao_ting_by_four_cards()
    ...
    def dealer_turn(self):
        ...
        if not dealer:  # 首局随机庄
            if self.__owner:
                p = self.get_player_by_uid(self.__owner)
                if p:
    ...
    async def __start_bao_ting_by_four_cards(self):
        """ 四张牌报听 """
        self.set_flow_status(FlowStatus.T_IN_FOUR_BAO_TING)
        for _ in range(self.__four_card_bao_ting):
            for p in self.seats:
                c = self.poker.pop()
                p.rev_card(c)
    ...
    async def deal_bu_pai(self):
        ...
        self.start_tian_ting() if self.__bao_ting else self.call_flow(0, self.enter_mo_pai_call)
    ...
    async def start_exchange_three(self):
        ...
        one_of_model = s2c_one_of_model()
        one_of_model.seconds = 15
        await self.inner_broadcast(CmdRoom.START_EXCHANGE_CARDS, one_of_model)
    ...
    async def __mo_pai(self, choice_seat=None, after_gang=None):
        ...
        if self.poker.left_count <= const.LIU_JU_COUNT or \
                (self.__is_fixed_bird and self.poker.left_count <= self.__bird_count):  # 黄庄了
    ...
    def judge_is_bi_hu(self, operates: list):
        """ 最后三张必胡，有其它操作时 """
    ...
    async def enter_mo_pai_call(self):
        ...
        data["is_bi_hu"] = 1 if self.judge_is_bi_hu(operates) else 0
    ...
    def calc_operates_after_mo_pai(self,p:Player):
        ...
        if self.__bao_ting and not p.all_chu_cards and p.can_tian_ting > -1 and p.seat_id != self.dealer_id:
            if self.can_select_tian_ting(p,14):
        ...
        is_an_gang, gang_card_list = p.can_an_gang(Rule) #提前验证是否能暗杠
        if p.card_is_lock():
            ...
            if is_an_gang:
                ...
                for gang_card in valid_gang_cards:
                    ...
                    # 听牌一致的话可以杠
                    ting_list1 = Rule.get_ting_hu_list([],temp_cards,allow_hu_map)
    ...
    def can_operates_gang(self, p:Player):
        """判断是否有人能杠"""
    ...
    def is_jue_zhang(self, p:Player) -> bool:
        """
        params: cards 手牌
        ...
        for _p in self.seats:
            if _p.seat_id == p.seat_id:
                continue
            unknown_cards.extend(_p.cards)
            unknown_cards.extend(_p.zi_mo_cards)
        ...
        if len(p.cards) % 3 == 2:
            try:
                cards = [c for c in p.cards if c != curr_card]
            except ValueError:
                return False
        else:
            cards = p.cards.copy()
    ...
    async def turn_to_player_chu_pai(self, p: Player, is_header=False, after_peng=False):
        ...
        if is_header:
            seconds = TimerDelay.FIRST_CALL_SECONDS
            self.set_flow_status(FlowStatus.T_IN_DI_HU_CHU_PAI)
        else:
            seconds = TimerDelay.CALL_SECONDS
            self.set_flow_status(FlowStatus.T_IN_CHU_PAI)
        data = {"seat_id": p.seat_id, "seconds": seconds}
        data_model = S2CTurnToMahjong.pb_model(**data)
        await self.inner_broadcast(CmdRoom.TURN_TO,data_model,exclude_uid = p.uid)
        if p.card_is_lock():
            if self.__hu_pai_ti_shi and (p.all_chu_cards or len(p.cards)==5):
    ...
    def robot_play_card_by_suo_pai(self,p):
        """ 机器人锁牌后的打牌 """
        out_cards = p.get_out_not_lock_card()
        if len(out_cards) == 0:
            return
        code,_=  self.on_player_chu_pai(p, out_cards[0])
    ...
    async def enter_chu_pai_call(self):
        ...
        data = {
            ...
            "left_count": self.poker.left_count,
            "trustee": chu_pai_player.trustee,
        }
        for p in self.seats:
            if p.seat_id == self.curr_seat_id:
                operates = []
                p.operates = operates
                data["hand_cards"] = chu_pai_player.cards or []
            else:
                data["hand_cards"] = []
            data_model = S2CPlayCardsMahjong.pb_model(**data)
            await self.inner_send(p, CmdRoom.PLAY_CARDS, data_model)
            if p.seat_id != self.curr_seat_id:
        ...
        if self.can_operates():
            ...
            return
        self.__chong_feng_ji and self.deal_first_ji(chu_pai_player)
        ...
        for p in self.seats:
            ...
            if p.can_operates() and not self.has_do_action(p):
                ...
                if p.is_action_in_operates(ActionType.ACTION_TYPE_HU):
                    if self.flow_status_is_equal(FlowStatus.T_IN_ZHUAN_WAN_GANG_PAI_CALL): #抢杠胡漏胡
                        flag = False
                        self.record_lou_hu(p)
                    elif len(self.__gang_hou_chu_pai) > 0: #杠上炮漏胡
                        flag = False
                        self.record_lou_hu(p)
                if flag and p.card_is_lock() and p.can_hu_men_jian():
                    self.record_lou_hu(p)
                    # 一炮多响时有人选择过则这里-1
                ...
                one_of_model = s2c_one_of_model()
                one_of_model.seat_id = self.curr_seat_id
                await self.inner_send(CmdRoom.PLAYER_PASS, one_of_model)
    ...
    def check_action_end(self):
        ...
        if max_operate_list[0] == ActionType.ACTION_TYPE_HU:
            ...
            return self.somebody_hu(hu_list)
        if not is_finish:
            return
    ...
    def somebody_hu(self, hu_list: list) -> bool:
        self.__curr_card_exist = 0
        self.hu_da_notify(hu_list)
        self.__winner_list = [self.get_player_by_seat_id(seat) for seat in hu_list]
        self.round_over(hu_list, OverType.HU_KAI)
        return True

    def somebody_men(self, hu_list: list):  # 三人均操作后判断有没有人胡牌
        self.__curr_card_exist = 0
        self.men_da_notify(hu_list)
        seat_id = hu_list[0]
        ...
        if len(p.ting_list) == 0:
            ting_list = Rule.get_ting_hu_list([],p.cards,allow_hu_map)
    ...
    def somebody_jian(self, hu_list: list):  # 三人均操作后判断有没有人胡牌
        """
        炸捡与捡都走这个逻辑，同时捡
        """
        self.__curr_card_exist = 0
        self.jian_da_notify(hu_list)
        allow_hu_map = {HuType.DI_LONG_QI: False, HuType.JIN_GOU_DIAO: False,
                        HuType.QI_DUI: True}
        # 捡完 锁定牌组 自动出牌
        ...
        if self.__jie_pao_count != 2 or len(hu_list) == 2:
    ...
    async def somebody_peng(self, peng_list: list) -> bool:  # 三人均操作后判断有没有人碰牌
        ...
        data = {"from_seat_id": self.curr_seat_id, "seat_id": p.seat_id, "is_finish": int(flag), "card": card}
        if flag:
            self.__curr_card_exist = 0
    ...
    def somebody_zhuan_wan_gang(self, gang_list: list) -> bool:
        if len(gang_list) != 1:
            return False
        ...
        if self.flow_status_is_equal(FlowStatus.T_IN_MO_PAI_CALL) and self.curr_player().mo_pai == card:
            self.__curr_card_exist = 0
        self.enter_zhuan_wan_gang_call(p, card)
        return True
    ...
    async def somebody_an_gang(self, gang_list: list) -> bool:
        ...
        if flag:
            ...
            if self.flow_status_is_equal(FlowStatus.T_IN_TIAN_TING):
                ...
                self.check_tian_ting_enter_next()
            else:
    ...
    async def zha_hu_da_notify(self, hu_list):
        ...
        for win_seat in hu_list:
            ...
            winner.hu_type = hu_info.get("hu_type", -1)
            data.append({
                "hu_base_info":{
                    "is_zha_hu": 1,
    ...
    async def enter_zhuan_wan_gang_call(self, curr_player, card):
        ...
        for seat_id in self.seats:
            ...
            data = {
                ...
                "operates": operates,
                "is_bi_hu": 1 if self.__hu_pai_ti_shi and ActionType.ACTION_TYPE_HU in operates else 0
            }
        ...
        if self.can_somebody_hu():  # 有人可以胡，则需要等待
            ...
            return
        self.do_zhuan_wan_gang_end()


    def do_zhuan_wan_gang_end(self):
        curr_player = self.__curr_action_player
        self.__gang_hou_mo_pai = []  # 有人转弯杠则连续杠被打断
        # 玩家 do 转弯杠
        flag = curr_player.zhuan_wan_gang(self.__curr_card)
    ...
    async def check_can_gang_after_peng(self, p):
        ...
        if can_an_gang or can_zwg:
            ...
            if can_an_gang and can_zwg:
                operates = [ActionType.ACTION_TYPE_AN_GANG, ActionType.ACTION_TYPE_ZHUAN_WAN_GANG]
            elif can_an_gang:
                operates = [ActionType.ACTION_TYPE_AN_GANG]
                data["can_gang_list"] = gang_card_list
            else:
                operates = [ActionType.ACTION_TYPE_ZHUAN_WAN_GANG]

            p.operates = operates
            data["operates"] = p.operates
            opt_model = S2CPublicOperatesMahjong.pb_model(**data)
            await self.inner_send(p, CmdRoom.PUBLIC_OPERATES,opt_model)
            result = {"seat_id": p.seat_id, "seconds": seconds}
    ...
    async def men_da_notify(self, hu_list):
        ...
        for seat_id in hu_list:
            ...
            other_model = S2CHuBaseInfo.pb_model(**other_data)
    ...
    def get_zha_hu_type(self, p, is_over=False):
        ...
        gang_card = 0  # 记录抢杠|杠上炮时的杠，如果是默认鸡后续算分时需要+鸡的分
        if is_zi_mo:
            if len(self.__gang_hou_mo_pai) > 0:  # 杠后摸牌
                extra_fan.append(ExtraHuPai.GANG_SHANG_HUA)  # todo: 这里主要处理漏闷，后续需要调整
    ...
    def get_player_cards_info(self, hu_list=None, is_zi_mo=False):
        ...
        for p in self.seats:
            if not is_zi_mo and hu_list and p.seat_id in hu_list:
                p.rv_card(self.__curr_card)  # 非自摸将当前牌放到玩家手牌中
    ...
    def is_bi_hu(self, operates: list):
        """ 最后三张必胡，有其它操作时 """
        if self.poker.left_count < const.XUE_LIU_LEFT_BI_HU:
            return True
        if not self.__hu_pai_ti_shi:
    ...
    def calc_operates_after_chu_pai(self,p:Player):
        ...
        if can_hu and len(self.__gang_hou_chu_pai) > 0:
            return result
        is_ming_gang,card = self.ming_gang_de_qi(p, self.__curr_card)
        if p.card_is_lock():
            if is_ming_gang:
                allow_hu_map = {HuType.DI_LONG_QI:self.__di_long_qi,HuType.JIN_GOU_DIAO:False,
                                HuType.QI_DUI:True}
                temp_cards = [c for c in p.cards if c != card]
                ting_list1 = Rule.get_ting_hu_list([],temp_cards,allow_hu_map)
                # 一致的话可以杠
            ...
            if is_ming_gang:
                result.append(ActionType.ACTION_TYPE_MING_GANG)
            if p.can_peng(card, Rule):
                result.append(ActionType.ACTION_TYPE_PENG)

        if not self.__hu_pai_ti_shi and not p.is_robot:
    ...
    def get_base_score(self, hu_info: dict, extra_hu_lst, seat_id=-1, zi_mo=False) -> int:
        ...
        base_score = const.PAI_XING_SCORE_MAP[hu_type]
    ...
    def cal_suo_de_jia_1_gang_is_ji(self, gang_cards) -> int:
        ...
        for gang_card in gang_cards:
            ...
            score = JI_PAI_SCORE.get(card)  # 单个鸡的分
    ...
    def deal_men_jian_data(self, p:Player, win_total_score, hu_info, check_type=CheckType.CHECK_MEN):
        data = {
            "card": p.mo_pai if hu_info.get("is_zi_mo") else self.__curr_card,
    ...
    async def on_player_pass(self,p:Player):
        ...
        if self.flow_status_is_equal(FlowStatus.T_IN_TIAN_TING) and self.dealer_id == p.seat_id and p.cards_len==14:
            p.can_tian_ting = -1
    ...
    async def on_player_chu_pai(self,player:Player,data):
        gang_model.ParFromString(data)
        card = gang_model.card or 0
        if not self.room_status_is_equal(RoomStatus.T_PLAYING):
            return StaCode.RULE_ERR,"桌子状态不在游戏中，不可出牌"
        if self.flow_status not in (FlowStatus.T_IN_CHU_PAI, FlowStatus.T_IN_DI_HU_CHU_PAI,
                                    FlowStatus.T_IN_MO_PAI_CALL):
            return StaCode.FLOW_ERR,"当前流程不在可出牌流程"
        ...
        if player.can_hu_men_jian:
            ...
            if player.is_action_in_operates(ActionType.ACTION_TYPE_JIAN):
                player.zha_type = 1
        ...
        if player.tian_ting or len(player.men_cards) > 0:
            ...
            if len(player.ting_list) == 0:
                allow_hu_map = {HuType.DI_LONG_QI:False,HuType.JIN_GOU_DIAO:False,
                                HuType.QI_DUI:True}
                ting_list = Rule.get_ting_hu_list([],player.cards,allow_hu_map)
    ...
    async def on_player_peng(self,player:Player,data):
        ...
        if ActionType.ACTION_TYPE_HU in player.operates or ActionType.ACTION_TYPE_JIAN in player.operates:
            self.set_tui_zhang_ke_kai(player)
    ...
    async def on_player_gang(self,player:Player,data):
        ...
        if gang_act in (ActionType.ACTION_TYPE_ZHUAN_WAN_GANG, ActionType.ACTION_TYPE_AN_GANG):
            gang_model.ParsFromString(data)
            card = gang_model.card or 0
    ...
    async def on_player_jian(self, player:Player, data):
        ...
        if self.flow_status not in (FlowStatus.T_IN_PUBLIC_OPRATE,FlowStatus.T_IN_ZHUAN_WAN_GANG_PAI_CALL,
                                      FlowStatus.T_IN_TIAN_HU, FlowStatus.T_IN_TIAN_TING,FlowStatus.T_IN_MING_GANG_PAI_CALL):
    ...
    async def on_player_shang_ga(self, player:Player, data):
        shang_ga_model.ParsFromString(data)
    ...
    async def on_player_exchange_cards(self,player:Player,data):
        ...
        exchange_model.ParsFromString(data)
        ...
        if self.exchange_cards_is_end():
            ...
            result = {}
            for p in self.seats:
                get_cards = p_get_cards.get(p.seat) or []
    ...
    async def on_player_tian_ting(self,p:Player,data):
        tian_ting_model.ParsFromString(data)
        is_tian_ting = tian_ting_model.is_tian_ting or 0
        if self.flow_status not in (FlowStatus.T_IN_TIAN_TING, FlowStatus.T_IN_FOUR_BAO_TING, FlowStatus.T_IN_MO_PAI_CALL):
        ...
        if len(p.cards) == 14 and p.is_action_in_operates(ActionType.ACTION_TYPE_HU)():
            ...
            # 设置ting_list
            ting_list = Rule.get_ting_hu_list(p.table_cards,p.cards,allow_hu_map)
            p.ting_list = ting_list
    ...
    def check_tian_ting_enter_next(self):
        if self.flow_status_is_equal(FlowStatus.T_IN_TIAN_TING):
            return
    ...
    def calc_operates_in_tian_ting(self, p: Player, dealer=False):
        ...
        if not p:
            return result
        if not dealer:
            if self.can_select_tian_ting(p,13):
                result.append(ActionType.ACTION_TYPE_TIAN_TING)
        else:
            if self.can_select_tian_ting(p,14):
                result.append(ActionType.ACTION_TYPE_TIAN_TING)
    ...
    @staticmethod
    def can_select_tian_ting(p:Player,cards_len,mo_pai=0):
        if p.cards_len !=cards_len:
            return False
        if cards_len==13 and p.chu_pai_len() == 0 and p.can_tian_ting >= 0:
            if Rule.r_can_tian_ting(p.table_cards, p.cards, p.que):
                return True
        if cards_len==14 and p.table_cards_len == 0 and p.can_tian_ting >= 0:
    ...
    def get_hu_type(self, p:Player, dian_pao=False, only_calc_hu=False, lou=False):
        ...
        if is_sha_bao:
            if ExtraHuPai.TIAN_HU not in extra_fan or len(p.cards) != 14:  # 庄起手的14张天胡，闲家报听无效则无杀报
                extra_fan.append(ExtraHuPai.SHA_BAO)
        ...
        is_qing_yi_se = Rule.has_hu_is_qing_yi_se(deepcopy(p.table_cards), deepcopy(p.cards), self.__curr_card)
    ...
    def find_target_exchange_cards_seat(self, player):
        for i in range(player.seat_id + 1, self.max_player_count):
            p = self.seats[i]
            if p.tian_ting:
                continue
            return p
        for i in range(1, player.seat_id):
            p = self.seats[i]
            if p.tian_ting:
                continue
            return p
    ...
    def do_check_out(self, over_type=OverType.DEFAULT):
        ...
        if self.__do_check_out_draw(accounts, is_liu_ju):
            ...
            return accounts, bird_list[0] if bird_list else -1

        if self.max_player_count == 2:
            if is_liu_ju:
                return self.liu_ju_check_out_by_num_2(accounts), -1
            return self.kai_pai_check_out_num_2(accounts)

        # 流胡或胡开
        if is_liu_ju:
            return self.liu_ju_check_out(accounts), -1
    ...
    def check_ze_ren_ji(self, accounts, liu_ju=False, is_bao=False):
        ...
        if self.__ze_ren_ji_seat_id != 0 and self.__ze_ren_ji_win_seat_id != 0:
            ji_card = CardsType.YAO_JI
            key = JiType.ZE_REN_JI
            score = JI_PAI_SCORE.get(key, 0)
            self.concreteness_check_ze_ren_ji(
                accounts, self.__ze_ren_ji_win_seat_id, self.__ze_ren_ji_seat_id, ji_card, score, liu_ju, is_bao)
        ...
        if self.__ze_ren_wgj_seat_id != 0 and self.__ze_ren_wgj_win_seat_id != 0:
            ...
            score = JI_PAI_SCORE.get(key, 0)
            self.concreteness_check_ze_ren_ji(
                accounts, self.__ze_ren_wgj_win_seat_id, self.__ze_ren_wgj_seat_id, ji_card, score, liu_ju, is_bao)

    def check_ji(self, accounts, fan_bird_list=None, is_bao=True, liu_ju=False):
        """ 结算鸡分 """
        double_bao = self.__double_bao and is_bao
        fan_bird_list = fan_bird_list or set()
        ...
        for p in self.seats:
            ...
            if p.seat_id in self.__shao_ji_gang_seats:
                continue
            p.calc_all_ji_pai(self.__default_ji, fan_bird_list, self.__man_tang_ji)  # 计算玩家有几个鸡牌
                ...
                if ji in self.__default_ji:
                    ...
                    if ji in fan_bird_list:
                        ...
                        if pg_ji_count > 0:
                            ...
                            score = JI_PAI_SCORE.get(ji, 1) * pg_ji_count * 2  # 金鸡 x 2
                    ...
                    if self.__zhan_ji:
                        ...
                        if stand_ji_count > 0:
                            ...
                            # x2是站鸡翻倍
                            score += JI_PAI_SCORE.get(ji, 1) * stand_ji_count * bei_lv * 2
                if bearer:
                    # x2是炸胡者承担2份
                    if ji == CardsType.WU_GU_JI and ji not in self.__default_ji:
                        score = (score + count) * 2
                    else:
                        score = (score + JI_PAI_SCORE.get(ji, 1) * count) * 2

                    if double_bao:
                        score *= 2

                    other_data = self.other_ming_xi_data(type_, p.seat_id, -score, ji, get_bearer=get_bearer.seat_id)
                    self.update_result_score(accounts, bearer.seat_id, 0, other_data)

                    self_data = self.self_ming_xi_data(type_, [bearer.seat_id], score, ji)
                    ...
                    if ji == CardsType.WU_GU_JI and ji not in self.__default_ji:
                        ...
                        per_score = score + JI_PAI_SCORE.get(ji, 1) * count
                    for other_p in self.seats:
                        if p.seat_id == other_p.seat_id:
                            continue
                ...
                for data in p.table_cards:
                    ...
                    if bearer and bearer.seat_id != pei_seat:
                        other_data = self.other_ming_xi_data(
                            type_, p.seat_id, -per_score, card, get_bearer=get_bearer.seat_id)
                        self_data = self.self_ming_xi_data(type_, [bearer.seat_id], per_score, card)
                        self.update_result_score(accounts, bearer.seat_id, 0, other_data)
                        self.update_result_score(accounts, p.seat_id, 1, self_data)
                    else:
                        ...
                        self_data = self.self_ming_xi_data(type_, [pei_seat], per_score, card)
        ...
        if p.is_zha_hu:
            ...
            if p.jiao_pai > 0:
                ...
                if bearer:
                    ...
                    score *= 2
                    other_data = self.other_ming_xi_data(
                        type_, cfj_seat_id, -score, card, get_bearer=get_bearer.seat_id)
                    self_data = self.self_ming_xi_data(type_, [bearer.seat_id], score, card)
                ...
                if bearer and get_bearer.jiao_pai > 0:
                    other_data = self.other_ming_xi_data(
                        type_, get_bearer.seat_id, -score, card, get_bearer=get_bearer.seat_id)
                    self_data = self.self_ming_xi_data(type_, [bearer.seat_id], score, card)

                    self.update_result_score(accounts, bearer.seat_id, 0, other_data)
                    self.update_result_score(accounts, get_bearer.seat_id, 1, self_data)
                else:
                    win_from = []
                    total_score = 0
                    for other_p in self.seats:
    ...
    def concreteness_check_ze_ren_ji(self, accounts, ze_ren_win, ze_ren_lose, card, score, liu_ju, is_bao):
        ...
        if get_zrj_p.is_zha_hu:
            ...
            self_data = self.self_ming_xi_data(type_, [ze_ren_lose], -score, card)
            self.update_result_score(accounts, ze_ren_lose, 0, other_data)
            self.update_result_score(accounts, ze_ren_win, 1, self_data)
        else:
            ...
            if get_zrj_p.jiao_pai > 0:
                ...
                other_data = self.other_ming_xi_data(type_, ze_ren_win, -score, card, get_bearer=get_bearer_seat)
                self_data = self.self_ming_xi_data(type_, [pei_seat], score, card)
                self.update_result_score(accounts, pei_seat, 0, other_data)
                self.update_result_score(accounts, ze_ren_win, 1, self_data)
            else:
                # 责任鸡玩家需要叫牌且未炸胡才能得到赔分
    ...
    def liu_ju_check_out_by_num_2(self, accounts: dict):
        """ 2人流局结算 """
        ...
        for p in self.seats:
            ...
            if p.jiao_pai <= 0:
                continue
            key = p.jiao_pai
            score = const.PAI_XING_SCORE_MAP[key]
            type_ = CheckType.CHECK_LIU_JU_CHA_JIAO
            # 内循环是炸胡或者为叫牌玩家
            for other_p in self.seats:
                if p.seat_id == other_p.seat_id:
                    continue
                if not other_p.is_zha_hu and other_p.jiao_pai > 0:
                    continue
                if other_p.is_zha_hu:
                    is_gt, score = self.__compare_paixing_is_gt_qing_yi_se(key)

                if p.tian_ting == 1:
                    if p.jiao_pai == HuType.PING_HU:
                        score = const.PAI_XING_SCORE_MAP[HuType.QING_YI_SE]
                    else:
                ...
                lose_data = self.other_ming_xi_data(type_, p.seat_id, score=-score)
                self.update_result_score(accounts, other_p.seat_id, 0, lose_data)

                # 获得查叫分者
                win_data = self.self_ming_xi_data(type_, [other_p.seat_id], score=score)
                self.update_result_score(accounts, p.seat_id, 1, win_data)

        # 2.闷捡分
        self.check_men_jian_by_num_2(accounts, liu_ju=True)
    ...
    def kai_pai_check_out_num_2(self, accounts: dict):
        """ 2人开牌结算 """
        self.kai_hu_cha_jiao(accounts)
        # 翻鸡
        fan_ji_cards, zhuo_ji = self.calc_fan_ji_cards()
        self.__ji_cards = fan_ji_cards
        # 1.开牌结算
        for hu_info in self.__kai_pai_hu_info:
            self.check_by_num_2(accounts, hu_info)

        # 2.闷捡结算
        self.check_men_jian_by_num_2(accounts)
        # 3.结算鸡分
        # 3.1 冲锋鸡
        self.check_chong_feng_ji(accounts, fan_ji_cards)
        ...
        # 3.31 先算正常玩家的鸡分（叫牌+未炸胡）
        # 外循环控制叫牌玩家，内循环控制其余所有玩家
        self.check_ji(accounts, fan_bird_list)
    ...
    def liu_ju_check_out(self, accounts: dict):
        ...
        if 0 < len(no_zha_hu_and_jiao_players) < self.__max_player_count:
                ...
                if flag:
                    ...
                    if p1.tian_ting == 1:
                        if p1.jiao_pai == HuType.PING_HU:
                            one_score = const.PAI_XING_SCORE_MAP[HuType.QING_YI_SE]
                        else:
                            # 如果p叫大对子 + 报听，炸胡玩家包清一色 + 报听 = 双清
                            one_score += const.PAI_XING_SCORE_MAP[HuType.QING_YI_SE]
                    ...
                    # 炸胡者自己承担>=清一色
                    is_gt, two_score = self.__compare_paixing_is_gt_qing_yi_se(key)
                    if p1.tian_ting == 1:
                        if p1.jiao_pai != HuType.PING_HU:
                            # 如果p叫大对子 + 报听，炸胡玩家包清一色 + 报听 = 双清
                            two_score += const.PAI_XING_SCORE_MAP[HuType.QING_YI_SE]
    ...
    def kai_pai_check_out(self, accounts: dict):
        ...
        统一数据结构
        """
        # self.__kai_hu_cha_jiao(accounts)
        # 翻鸡
        fan_ji_cards, zhuo_ji = self.calc_fan_ji_cards()
        self.__ji_cards = fan_ji_cards
        self.kai_hu_cha_jiao(accounts)
        ...
        # 4.2 结算包杠
        # if self.__bao_gang:
        #     self.__bao_gang_check(accounts)
    ...
    def bao_ji_check(self, accounts):
        """
        结算包鸡
        只有炸胡/流局未听牌玩家需要包鸡
        ...
        for p in self.seats:
            ...
            if p.jiao_pai > 0 and not p.is_zha_hu:
                continue
            p.get_bao_ji(self.__default_ji)  # 计算玩家有几个鸡牌
            p_ji_cards = p.ji_pai[:]  # list
            p_stand_ji = p.calc_stand_ji(self.__default_ji)
            # 冲锋鸡之前算过 -1
            if self.__chong_feng_ji_seat_id != 0 and self.__chong_feng_ji_seat_id == p.seat_id:
            ...
            if p.is_zha_hu:
                ...
                if mg_count > 0:
                    ...
                    for data in p.table_cards:
                        ...
                        if bearer:
                            other_data = self.other_ming_xi_data(
                                type_, bearer.seat_id, per_score, card, get_bearer=get_bearer.seat_id)
                            self_data = self.self_ming_xi_data(type_, [get_bearer.seat_id], -per_score, card)
                            self.update_result_score(accounts, bearer.seat_id, 0, other_data)
                            self.update_result_score(accounts, get_bearer.seat_id, 1, self_data)
                        else:
                            other_data = self.other_ming_xi_data(type_, p.seat_id, per_score, card)
    ...
    @staticmethod
    def __compare_paixing_is_gt_qing_yi_se(hu_type, is_zi_mo=False):
        """
        牌型低于清一色算清一色，牌型高于清一色算实际牌型
        此接口只判断玩家牌型是否小于清一色
        """
        paixing_score = const.PAI_XING_SCORE_MAP.get(hu_type, 0)
        qingyise_score = const.PAI_XING_SCORE_MAP.get(HuType.QING_YI_SE)
        flag = True
        if paixing_score < qingyise_score:
            flag = False
            paixing_score = qingyise_score
        if is_zi_mo:
            paixing_score *= 2
        return flag, paixing_score

    def check_men_jian_by_num_2(self, accounts: dict, liu_ju=False):
        """
        2人场结算闷捡,2人场只有赔付者与获得者
        结算闷捡，流局和正常开牌统一为一个接口
        ...
        for men_info in self.__men_record:
            self.check_by_num_2(accounts, men_info, liu_ju)
    ...
    def check_men_jian(self, accounts: dict, liu_ju=False):
        ...
        """
        for men_info in self.__men_record:
            self.__check_by_num_3(accounts, men_info, liu_ju=liu_ju)


    def check_by_num_2(self, accounts, hu_info, liu_ju=False):
        """
        处理2人结算的情况
        """
        ...
        base_score = self.get_base_score(hu_info, extra_hu_lst, zi_mo=is_zi_mo)
        ...
        for p in self.seats:
            if p.seat_id != seat_id:
                pei_p = p
                break
        pei_seat = pei_p.seat_id
        extra_score = self.cal_extra_hu_score(hu_info, extra_hu_lst, pei_seat, is_zi_mo)
        ...
        if winner.is_zha_hu:
            ...
            if pai_xing_score != 0:
                _, pai_xing_score = self.__compare_paixing_is_gt_qing_yi_se(hu_type, is_zi_mo)

            total_score = pai_xing_score + extra_score
            other_data = self.other_ming_xi_data(check_type, seat_id, total_score, card, hu_type, extra_hu_lst)
            self_data = self.self_ming_xi_data(check_type, [pei_seat], -total_score, card, hu_type, extra_hu_lst)
            self.update_result_score(accounts, pei_seat, 0, other_data)
            self.update_result_score(accounts, seat_id, 1, self_data)

            if self.__shang_ga:
                shang_ga_score = winner.shang_ga_score + pei_p.shang_ga_score
                check_type_ = CheckType.CHECK_GU_MAI  # 估卖
                other_data = self.other_ming_xi_data(check_type_, seat_id, shang_ga_score, card)
                self_data = self.self_ming_xi_data(check_type_, [pei_seat], -shang_ga_score, card)
                self.update_result_score(accounts, pei_seat, 0, other_data)
                self.update_result_score(accounts, seat_id, 1, self_data)
        else:
            # 流局且未炸胡且叫牌者不用赔
            if liu_ju and not pei_p.is_zha_hu and pei_p.jiao_pai > 0:
                return
            # 当winner未炸胡，炸胡者需要承担未炸胡者的损失
            pai_xing_score = base_score
            total_score = pai_xing_score + extra_score
    ...
    def __check_by_num_3(self, accounts, hu_info, liu_ju=False):
        """
        处理3人胡牌|闷捡的结算，统一接口
        """
        card = hu_info.get("card")
        ...
        if not is_zi_mo:
            ...
            if winner.is_zha_hu:
                ...
                pai_xing_score = base_score
                if pai_xing_score != 0:
                    _, pai_xing_score = self.__compare_paixing_is_gt_qing_yi_se(hu_type)
                total_score = pai_xing_score + extra_score
                ...
                # 流局且未炸胡且叫牌者不用赔
                # if liu_ju and not fang_pao_p.is_zha_hu and fang_pao_p.jiao_pai > 0:
                #     return
                # 当winner未炸胡，炸胡者需要承担未炸胡者的损失
                ...
                if bearer:
                    ...
                    if bearer != fang_pao_p:
                        ...
                        if base_score != 0:
                            is_gt, pai_xing_score = self.__compare_paixing_is_gt_qing_yi_se(hu_type)

                total_score = pai_xing_score + extra_score
                other_data = self.other_ming_xi_data(
                    check_type, seat_id, -total_score, card, hu_type, extra_hu_lst, get_bearer_seat)
                self_data = self.self_ming_xi_data(check_type, [pei_seat], total_score, card, hu_type, extra_hu_lst)
                self.update_result_score(accounts, pei_seat, 0, other_data)
                self.update_result_score(accounts, seat_id, 1, self_data)
                ...
                if self.__shang_ga:
                    ...
                    check_type_ = CheckType.CHECK_GU_MAI  # 估卖
                    other_data = self.other_ming_xi_data(
                        check_type_, seat_id, -shang_ga_score, card, get_bearer=get_bearer_seat)
                    self_data = self.self_ming_xi_data(check_type_, [pei_seat], shang_ga_score, card)
                    self.update_result_score(accounts, pei_seat, 0, other_data)
                    self.update_result_score(accounts, seat_id, 1, self_data)
        else:
            ...
            if winner.is_zha_hu:
                ...
                for p in self.seats:
                    ...
                    pai_xing_score = base_score
                    if pai_xing_score != 0:
                        _, pai_xing_score = self.__compare_paixing_is_gt_qing_yi_se(hu_type, is_zi_mo=True)
                ...
                if liu_ju:
                    ...
                    if status == PlayerStatusType.JIAO_2:  # 其余2个都叫牌
                        ...
                        self_data = self.self_ming_xi_data(
                            check_type, [pei_seat], total_score, card, hu_type, extra_hu_lst)
                        other_data = self.other_ming_xi_data(
                            check_type, seat_id, -total_score, card, hu_type, extra_hu_lst)
                        self.update_result_score(accounts, seat_id, 1, self_data)
                        self.update_result_score(accounts, pei_seat, 0, other_data)
                        if self.__shang_ga:
                            pei_p = self.get_player_by_seat_id(pei_seat)
    ...
    def check_zha_hu_bear_no_zha_hu(
        ...
        if bearer:
            ...
            other_data = self.other_ming_xi_data(
                _type, winner.seat_id, -total_score, card, men_info.get("hu_type"), extra_hu_lst, get_bearer.seat_id
            )
            self_data = self.self_ming_xi_data(
                _type, [bearer.seat_id], total_score, card, men_info.get("hu_type"), extra_hu_lst)
            self.update_result_score(accounts, bearer.seat_id, 0, other_data)
            ...
            if self.__shang_ga:
                ...
                other_data = self.other_ming_xi_data(
                    check_type, winner.seat_id, -shang_ga_score, card, get_bearer=get_bearer.seat_id)
                self_data = self.self_ming_xi_data(check_type, [bearer.seat_id], shang_ga_score, card)
                self.update_result_score(accounts, bearer.seat_id, 0, other_data)
                self.update_result_score(accounts, winner.seat_id, 1, self_data)
