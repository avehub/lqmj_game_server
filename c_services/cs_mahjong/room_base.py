...
class Room(BaseCardRoom):
    ...
    async def turn_to_player_chu_pai(self, p: Player, after_peng=False):
        ...
        if p.card_is_lock():
            ...
            if self.__hu_pai_ti_shi and (p.all_chu_cards or p.cards_len == 5):
    ...
    def cal_suo_de_jia_1_gang_is_ji(self, gang_cards) -> int:
        ...
        for gang_card in gang_cards:
            ...
            ji_pai_score = self.get_ji_pai_score_map()
    ...
    def get_hu_type(self, p: Player, dian_pao=False, only_calc_hu=False, lou=False):
        ...
        hu_type, hu_path = Rule.can_hu(
            table_cards, hand_cards, self.__curr_card, {HuType.QI_DUI: True, HuType.JIN_GOU_DIAO: not is_gy,
                                                        HuType.DI_LONG_QI: self.__di_long_qi, }, is_gy=is_gy)
        if not hu_type:
            return {}, False, []
        ...
        for p in self.seats:
            ...
            for ji, count in ji_card_count.items():
                ...
                if bearer:
                    ...
                    self.check_extra_ji(ji, score, count, p, liu_ju, double_bao, accounts, type_)

    def check_extra_ji(self, ji, score, count, p, liu_ju, double_bao, accounts, type_):
        win_total = 0
        win_from = []
        ji_pai_score = self.get_ji_pai_score_map()
        if ji == CardsType.WU_GU_JI and ji not in self.__default_ji:
            ...
            if mg_count > 0:
                for data in p.table_cards:
                    ...
                    if bearer and bearer.seat_id != pei_seat:
                        self.update_score(type_, p.seat_id, bearer.seat_id, -per_score, card, accounts, get_bearer.seat_id)
                    else:
        ...
        if p.is_zha_hu:
            ...
            if p.jiao_pai > 0:
                ...
                if bearer and get_bearer.jiao_pai > 0:
                    self.update_score(type_, get_bearer.seat_id, bearer.seat_id, -score, card, accounts, get_bearer.seat_id)
                else:
                    win_from = []
                    total_score = 0
                    for other_p in self.seats:
        ...
        if 0 < len(no_zha_hu_and_jiao_players) < self.max_player_count:
            ...
            for seat1 in no_zha_hu_and_jiao_players:
                ...
                if flag:
                    ...
                    # 炸胡者自己承担>=清一色
                    is_gt, two_score = self.__compare_paixing_is_gt_qing_yi_se(key, score_map=self.pai_xing_score_map)
                    if p1.tian_ting == 1:
                        if p1.jiao_pai != HuType.PING_HU:
                            # 如果p叫大对子 + 报听，炸胡玩家包清一色 + 报听 = 双清
                    ...
                    if not no_jiao_player and zha_hu_player:
                        is_gt, score = self.__compare_paixing_is_gt_qing_yi_se(key, score_map=self.pai_xing_score_map)
                        if p1.tian_ting == 1:
                            if p1.jiao_pai != HuType.PING_HU:
    ...
    def bao_ji_check(self, accounts):
        ...
        for p in self.seats:
            ...
            ji_card_count = self.cal_card_count(p_ji_cards)
            stand_ji_card_count = self.cal_card_count(p_stand_ji)
            print("stand_ji_card_count", stand_ji_card_count)
            # 未炸胡+未听牌包鸡
            ...
            if p.is_zha_hu:
                ...
                if mg_count > 0:
                    ...
                    for data in p.table_cards:
                        ...
                        if bearer:
                            self.update_score(type_, bearer.seat_id, get_bearer.seat_id, per_score, card, accounts, get_bearer.seat_id)
                        else:
                            self.update_score(type_, p.seat_id, pei_seat, per_score, card, accounts)
    ...
    @staticmethod
    def __compare_paixing_is_gt_qing_yi_se(hu_type, is_zi_mo=False, score_map=const.PAI_XING_SCORE_MAP):
        """
        牌型低于清一色算清一色，牌型高于清一色算实际牌型
        此接口只判断玩家牌型是否小于清一色
        """
        paixing_score = score_map.get(hu_type, 0)
        qingyise_score = score_map.get(HuType.QING_YI_SE)
        flag = True
    ...
    def check_by_num_3(self, accounts, hu_info, liu_ju=False):
        ...
        if not is_zi_mo:
            ...
            if winner.is_zha_hu:
                ...
                if pai_xing_score != 0:
                    _, pai_xing_score = self.__compare_paixing_is_gt_qing_yi_se(hu_type, score_map=self.pai_xing_score_map)
                ...
                if bearer:
                    ...
                    if bearer != fang_pao_p:
                        ...
                        if base_score != 0:
                            is_gt, pai_xing_score = self.__compare_paixing_is_gt_qing_yi_se(hu_type, score_map=self.pai_xing_score_map)

                total_score = pai_xing_score + extra_score
                self.update_score(check_type, seat_id, pei_seat, -total_score, card, accounts, get_bearer_seat, hu_type, extra_hu_lst)
            ...
            if winner.is_zha_hu:
                ...
                for p in self.seats:
                    ...
                    pai_xing_score = base_score
                    if pai_xing_score != 0:
                        _, pai_xing_score = self.__compare_paixing_is_gt_qing_yi_se(hu_type, True, self.pai_xing_score_map)
    ...
    def update_score(self, check_type_, seat_id, pei_seat, score, card, accounts, get_bearer=-1, hu_type=None, extra_hu_type=None):
    ...
    def check_zha_hu_bear_no_zha_hu(
        ...
        if bearer:
            ...
            if self.__shang_ga:
                ...
                self.update_score(check_type, winner.seat_id, bearer.seat_id, -shang_ga_score, card, accounts, get_bearer.seat_id)
