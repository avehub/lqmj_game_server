...
class Club(AdminAuthApi):
    async def post(self, req: Request, **kwargs):
        ...
        room_card = self.check_int(req.json.get("room_card"), require=True, p_name="茶馆基金")
    ...
    async def put(self, req: Request):
        ...
        room_card = self.check_int(req.json.get("room_card"), require=False, p_name="茶馆基金")
...
class ClubEvent(AdminAuthApi):
    async def get(self, req: Request, **kwargs):
        """茶馆事件"""
        club_id = self.check_int(req.args.get('club_id'), require=False, p_name='茶馆ID')
        uid = self.check_int(req.args.get('uid'), require=False, p_name='馆主ID(玩家ID)')
        type = self.check_int(req.args.get('type'), require=False, p_name='类型')
        page = self.check_int(req.args.get('page'), require=False, default=1, p_name='页码')
        page_size = self.check_int(req.args.get('page_size'), require=False, default=10, p_name='每页数量')
        data, msg = await ExtraClubEventRC.get_by_filter(uid=uid, club_id=club_id, event_type=type, page=page, page_size=page_size)
