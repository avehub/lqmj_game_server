import sys, os, asyncio, argparse, tracemalloc, datetime, atexit
...
def generate_memory_report():
    ...
    if tracemalloc.is_tracing():
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write(f"=== Memory Report - {datetime.datetime.now()} ===\n")
            f.write(f"Total allocated: {sum(stat.size for stat in snapshot.statistics('lineno')) / 1024:.2f} KB\n")
...
# 注册内存报告生成函数，确保程序退出时执行
atexit.register(generate_memory_report)
